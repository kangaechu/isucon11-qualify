...
func getIsuList(c echo.Context) error {
	...
	for _, isu := range isuList {
		...
		if foundLastCondition {
			conditionLevel := lastCondition.ConditionText
...
func postIsu(c echo.Context) error {
	...
	// TODO 新しいcharacterの種類が増える可能性がある。characterList変数を更新する必要がある！！
...
func getIsuGraph(c echo.Context) error {
	...
	resNew, err := generateIsuGraphResponseNew(tx, jiaIsuUUID, date)
	if err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}

	c.Logger().Infof("res: %#v", res)
	c.Logger().Infof("resNew: %#v", resNew)
...
// グラフのデータ点を一日分生成
func generateIsuGraphResponseNew(tx *sqlx.Tx, jiaIsuUUID string, graphDate time.Time) ([]GraphResponse, error) {
	dataPoints := []GraphDataPointWithInfo{}
	conditionsInThisHour := []IsuCondition{}
	timestampsInThisHour := []int64{}
	var startTimeInThisHour time.Time
	...
	for _, character := range characterList {
		...
		for _, isu := range isuList {
			...
			if len(conditions) > 0 {
				...
				conditionLevel := isuLastCondition.ConditionText
...
func postIsuCondition(c echo.Context) error {
	...
	for _, cond := range req {
		...
		conditionText, err := calculateConditionLevel(cond.Condition)
		if err != nil {
			c.Logger().Errorf("calculate condition error: %v", err)
			return c.NoContent(http.StatusInternalServerError)
		}
	...
	query_string := "INSERT INTO `isu_condition`" +
		"	(`jia_isu_uuid`, `timestamp`, `is_sitting`, `condition`, `message`, `condition_text`)" +
		"	VALUES (:jia_isu_uuid, :timestamp, :is_sitting, :condition, :message, :condition_text)"

	_, err = tx.NamedExec(query_string, isuconditions)

	err = tx.Commit()
	if err != nil {
		c.Logger().Errorf("db error: bulk insert: %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
